...
Stream<List<TarefasRecord>> queryTarefasRecord({
...
Future<List<TarefasRecord>> queryTarefasRecordOnce({
  Query Function(Query) queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<TarefasRecord>> queryTarefasRecordPage({
  Query Function(Query) queryBuilder,
...
Future<FFFirestorePage<ProfilePicRecord>> queryProfilePicRecordPage({
  Query Function(Query) queryBuilder,
  DocumentSnapshot nextPageMarker,
  int pageSize,
  bool isStream,
...
Future maybeCreateUser(User user) async {
  final userRecord = ProfilePicRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    return;
  }

  final userData = createProfilePicRecordData(
    email: user.email,
